...
# Confound regression
fn_res <- fn_deg
# Load 'gamlss' packages
if(!require(gamlss)) install.packages("gamlss"); library(gamlss)
# Remove columns with NA's
info <- info[,which(apply(info,2,function(x) sum(is.na(x)))==0)]
# Extract confounds to each connection
for(ii in 1:ncol(fn_deg)){
  # Set connection
  info$y <- fn_deg[,ii]
  # Apply model
  fit <- gamlss(y ~ lo(~PDSe)+Sex+FDRMS+Coil, data = info)
  # Store residuals
  fn_res[,ii] <- resid(fit)
}
...
if(0){
  ...
  for(ii in 1:length(cog_labs)){
    ...
    for(kk in 1:nrow(res)){
      ...
      info$y <- fn_res[,kk]
      # Set model
      grp_form <- as.formula(paste0(cog_labs[ii]," ~ s(y, k=4)"))
    ...
    if(length(edges)>0){
      ...
      outdir <- "Pubertal_Connectome/Cognition/NBS"
      if(!dir.exists(outdir)) dir.create(outdir, recursive = T)
      write.csv(fwe_list[[1]], file.path(outdir,outfile), quote = F, row.names = F)
      
      # Save NBS results (if pFWE < 0.05)
      if(!all(fwe_list[[1]][,2]>0.05)){
        # Create output directory
        fwe_dir <- file.path(outdir,cog_labs[ii])
        ...
        for(ff in fwe_comp){
          ...
          for(ee in sig_edge){
            ...
            gSC <- ggplot(data = info, mapping = aes(x = res)) + 
              ...
              theme_bw()
            # Save plot
            outfile <- paste0(plt_dir,"/",gsub("[.]","",colnames(fn_deg)[edge_idx]),".svg")
            ggsave(outfile, gSC, "svg")
          }#for(ee in sig_edge)
        }#for(ff in fwe_comp)
      }#if(!all(fwe_list[[1]][,2]>0.05))
    }#if(length(edges)>0)
...
nbs_files <- "https://raw.githubusercontent.com/BrainMapINB/Pubertal_Connectome/main/Cognition/NBS/CST_Score/CST_Score_comp1_edges.csv"
...
for(nn in 1:length(nbs_files)){
  ...
  for(ee in 1:length(edge_labs)){
    # Compute brain residuals
    info$res <- fn_res[,match(edge_labs[ee],colnames(fn_deg))]
  ...
  # Initialize the plot
  #outfile <- file.path(dirname(outdir),gsub("edges.csv","NBS-GAM.svg",basename(nbs_files[nn])))
  #svg(file = outfile, width = 4, height = 4)
  circos.clear()
